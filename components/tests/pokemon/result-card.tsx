...
export function ResultCard({ result, onRestart }: ResultCardProps) {
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: "Mein Pokémon Persönlichkeitstest Ergebnis",
        text: `Ich bin ${result.name}! Finde heraus, welches Pokémon du bist.`,
        url: window.location.href,
      })
    } else {
      // Fallback for browsers that don't support the Web Share API
      navigator.clipboard.writeText(
        `Ich bin ${result.name}! Finde heraus, welches Pokémon du bist: ${window.location.href}`,
      )
  ...
  return (
    <motion.div
      ...
      className="bg-white bg-opacity-80 backdrop-blur-sm rounded-lg overflow-hidden shadow-lg max-w-3xl mx-auto border-4 border-yellow-400"
    >
      <div className={`p-6 text-center ${result.color}`}>
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-2">Dein Ergebnis</h2>
        <p className="text-white mb-2">Typ: {result.type}</p>
      ...
      <div className="px-6 pb-6">
        <div className="flex flex-col md:flex-row items-center bg-blue-50 rounded-lg overflow-hidden -mt-6 border-2 border-blue-200">
          <div className="w-full md:w-1/3 relative h-64 md:h-auto">
            <Image src={result.image || "/placeholder.svg"} alt={result.name} fill className="object-contain p-4" />
          </div>
      ...
      <div className="bg-blue-50 p-6">
        ...
        <p className="text-gray-700 mb-4">
          Dieser Persönlichkeitstest basiert auf den Charaktereigenschaften beliebter Pokémon. Die Fragen wurden
          entwickelt, um deine Persönlichkeitsmerkmale mit denen der Pokémon zu vergleichen.
        </p>
        ...
        <div className="mt-6">
          <Link
            href="/tests"
            className="inline-block bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-lg transition-colors"
          >
