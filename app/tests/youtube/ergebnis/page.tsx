...
export async function generateMetadata({
  ...
  const youtuber = (currParams.youtuber as string) || "pewDiePie";
  ...
  if (!result) {
    return {
      ...
      description:
        "Finde heraus, welchem berühmten YouTuber deine Persönlichkeit am ähnlichsten ist!",
    };
  }
  ...
  return {
    ...
    description: `Laut unserem Test ähnelt deine Persönlichkeit ${
      result.name
...
export default async function YouTubeTestResultPage({ searchParams }: Props) {
  // youtuber Parameter aus der URL extrahieren
  const currParams = await searchParams;
  const youtuber = (currParams.youtuber as string) || "pewDiePie";
  ...
  const jsonLd = {
    ...
    "@type": "WebPage",
    name: `Du bist wie ${
      youtuberResults[youtuber as Youtuber]?.name || "PewDiePie"
    } | YouTuber Persönlichkeitstest`,
    description:
      "Finde heraus, welchem berühmten YouTuber deine Persönlichkeit am ähnlichsten ist!",
  };
